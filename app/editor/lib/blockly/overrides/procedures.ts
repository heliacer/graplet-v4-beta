/**
 * @override modified procedures by heliacer
 *
 * @license
...
export const procedureBlocks: { [key: string]: any } = {} // eslint-disable-line @typescript-eslint/no-explicit-any
...
const PROCEDURE_DEF_COMMON = {
  ...
  mutationToDom: function (
    ...
    for (let i = 0; i < this.argumentVarModels_.length; i++) {
      const parameter = utils.xml.createElement('arg')
  ...
  decompose: function (
    ...
    const containerBlockNode = utils.xml.createElement('block')
    containerBlockNode.setAttribute('type', 'procedures_mutatorcontainer')
    const statementNode = utils.xml.createElement('statement')
    statementNode.setAttribute('name', 'STACK')
    containerBlockNode.appendChild(statementNode)

    let node = statementNode
    for (let i = 0; i < this.arguments_.length; i++) {
      const argBlockNode = utils.xml.createElement('block')
      argBlockNode.setAttribute('type', 'procedures_mutatorarg')
      const fieldNode = utils.xml.createElement('field')
      fieldNode.setAttribute('name', 'NAME')
      const argumentName = utils.xml.createTextNode(this.arguments_[i])
      fieldNode.appendChild(argumentName)
      argBlockNode.appendChild(fieldNode)
      const nextNode = utils.xml.createElement('next')
  ...
  displayRenamedVar_: function (
    ...
    const mutator = this.getIcon(icons.MutatorIcon.TYPE)
...
procedureBlocks['procedures_defnoreturn'] = {
...
procedureBlocks['procedures_defreturn'] = {
  ...PROCEDURE_DEF_COMMON,
  /**
...
procedureBlocks['procedures_mutatorcontainer'] = PROCEDURES_MUTATORCONTAINER
...
procedureBlocks['procedures_mutatorarg'] = PROCEDURES_MUTATORARGUMENT
...
const PROCEDURE_CALL_COMMON = {
   ...
   */
  onchange: function (this: CallBlock, event: Events.Abstract) {
    ...
    if (
      ...
      if (!def) {
        ...
        field.appendChild(utils.xml.createTextNode(callName))
...
procedureBlocks['procedures_callnoreturn'] = {
...
procedureBlocks['procedures_callreturn'] = {
  ...PROCEDURE_CALL_COMMON,
  /**
...
const PROCEDURES_IFRETURN = {
   ...
   * @param e Move event.
   */
  onchange: function (this: IfReturnBlock, e: Events.Abstract) {
    if (
...
procedureBlocks['procedures_ifreturn'] = PROCEDURES_IFRETURN
